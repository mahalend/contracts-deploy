...
import { eNetwork, RewardsConfigInput } from "../../helpers/types";
...
const func: DeployFunction = async function ({
  ...
  console.log();
  console.log();
  console.log("*********** Incentive setup ***************");

  const network = process.env.FORK ? process.env.FORK : hre.network.name;
  const { incentivesRewardsVault, incentivesEmissionManager } =
    await getNamedAccounts();
  const incentivesEmissionManagerSigner = await hre.ethers.getSigner(
    incentivesEmissionManager
  );
  const incentivesRewardsVaultSigner = await hre.ethers.getSigner(
  ...
  const pullRewardsStrategyArtifact = await deployments.get(
    INCENTIVES_PULL_REWARDS_STRATEGY_ID
  );

  if (!isIncentivesEnabled(poolConfig)) {
    console.log(
      "[Warning] Skipping incentives config since it's disable in config"
    );
    return;
  }

  const incentivesConfig = poolConfig.IncentivesConfig;
  const rewards = getParamPerNetwork(
    incentivesConfig.rewards,
    network as eNetwork
  );
  if (!rewards) {
    console.log(
      "[Warning] skipping incentives configuration since rewards are not mentioned in incentive config"
    );
    return;
  }
  ...
  await Bluebird.mapSeries(Object.keys(rewards), async (symbol: string) => {
    ...
    if (isTestnet && address === ZERO_ADDRESS) {
      const testTokenArtifact = await deployments.get(
      ...
      const testToken = await hre.ethers.getContractAt(
        ...
        testTokenArtifact.address
      );
      console.log("- Approving pull strategy to access rewards");
      await waitForTx(
        await testToken
          .connect(incentivesRewardsVaultSigner)
          .approve(pullRewardsStrategyArtifact.address, MAX_UINT_AMOUNT)
      );
    }

    if (!address || address === ZERO_ADDRESS) {
      console.log(
        "[Warning] skipping incentives configuration for reward " +
          symbol +
          " since reward address is not mentioned or is 0x00..0"
      );
    } else {
      ...
      await waitForTx(
        await emissionManager
          ...
          .setEmissionAdmin(address, incentivesEmissionManager)
      );
    }
  });

  const rewardTokenOracles = getParamPerNetwork(
    incentivesConfig.rewardsOracle,
    network as eNetwork
  );
  const incentivesInput = getParamPerNetwork(
    incentivesConfig.incentivesInput,
    network as eNetwork
  );
  ...
  await Bluebird.mapSeries(
    incentivesInput as Array<any>,
    async (input: RewardsConfigInput) => {
      const params: {
        ...
        asset: (await deployments.get(`${input.asset}${ATOKEN_PREFIX}`))
          .address,
        reward: isTestnet
          ? (
              await deployments.get(
                `${input.reward}${TESTNET_REWARD_TOKEN_PREFIX}`
              )
            ).address
        ...
        rewardOracle: isTestnet
          ? (
              await deployments.get(
                `${input.reward}${TESTNET_PRICE_AGGR_PREFIX}`
              )
            ).address
