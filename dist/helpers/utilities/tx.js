...
exports.getAddressFromJson =
  exports.getProxyAdminBySlot =
  exports.getWalletBalances =
  exports.getContract =
  exports.deployContract =
  exports.getBlockTimestamp =
  exports.waitDeployment =
  exports.parseUnitsFromToken =
  exports.advanceTimeAndBlock =
...
const deployContract = async (contract, args, libraries, id) => {
  ...
  const artifact = await hre.deployments.deploy(
    id || `${contract}-${crypto_1.default.randomUUID()}`, // Prevent collisions with principal deployment in tests environment
    {
      contract,
      args,
      from,
...
const getWalletBalances = async () => {
  ...
  const accountTable = await bluebird_1.default.reduce(
    Object.keys(accounts),
    async (acc, accKey) => {
      acc.push({
        name: accKey,
        account: accounts[accKey],
        balance: (0, utils_1.formatEther)(
