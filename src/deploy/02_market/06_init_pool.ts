...
import { L2_ENCODER, L2_POOL_IMPL_ID, POOL_ADDRESSES_PROVIDER_ID, POOL_CONFIGURATOR_IMPL_ID, POOL_CONFIGURATOR_PROXY_ID, POOL_IMPL_ID, POOL_PROXY_ID } from "../../helpers/deploy-ids";
import { COMMON_DEPLOY_PARAMS, MARKET_NAME } from "../../helpers/env";
import { checkRequiredEnvironment, ConfigNames, isL2PoolSupported, loadPoolConfig } from "../../helpers/market-config-helpers";
...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...hre }: HardhatRuntimeEnvironment) {
    ...
    if (isPoolConfiguratorProxyPending) {
        const setPoolConfiguratorTx = await waitForTx(await addressesProviderInstance.setPoolConfiguratorImpl(poolConfiguratorImplDeployment.address));
