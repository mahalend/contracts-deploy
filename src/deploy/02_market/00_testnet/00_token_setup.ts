...
import { checkRequiredEnvironment, ConfigNames, isIncentivesEnabled, isProductionMarket, loadPoolConfig } from "../../../helpers/market-config-helpers";
import { FAUCET_ID, STAKE_AAVE_PROXY, TESTNET_REWARD_TOKEN_PREFIX, TESTNET_TOKEN_PREFIX } from "../../../helpers/deploy-ids";
import { eNetwork } from "../../../helpers/types";
import { deployInitializableAdminUpgradeabilityProxy, setupStkAave } from "../../../helpers/contract-deployments";

const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...DRE }: HardhatRuntimeEnvironment) {
    ...
    if (isIncentivesEnabled(poolConfig)) {
        ...
        const rewardSymbols = Object.keys(poolConfig.IncentivesConfig.rewards[network as eNetwork] || {});
        ...
        const testnetDeployment = Object.keys(allDeployments).filter((x) => x.includes(TESTNET_TOKEN_PREFIX));
        testnetDeployment.forEach((key) => console.log(key, allDeployments[key].address));
        console.log("Testnet Reward Tokens");
        console.log("======================");
        const rewardDeployment = Object.keys(allDeployments).filter((x) => x.includes(TESTNET_REWARD_TOKEN_PREFIX));
        rewardDeployment.forEach((key) => console.log(key, allDeployments[key].address));
        console.log("Native Token Wrapper WETH9", (await deployments.get(`${poolConfig.WrappedNativeTokenSymbol}${TESTNET_TOKEN_PREFIX}`)).address);
    }
