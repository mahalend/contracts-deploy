...
import { checkRequiredEnvironment, ConfigNames, getReserveAddresses, getTreasuryAddress, loadPoolConfig, savePoolTokens } from "../../helpers/market-config-helpers";
...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...hre }: HardhatRuntimeEnvironment) {
    ...
    const treasuryAddress = await getTreasuryAddress(poolConfig, network as eNetwork);
    const incentivesController = await deployments.get("IncentivesProxy");
    // Deploy Reserves ATokens
    await initReservesByHelper(ReservesConfig, reservesAddresses, ATokenNamePrefix, StableDebtTokenNamePrefix, VariableDebtTokenNamePrefix, SymbolPrefix, deployer, treasuryAddress, incentivesController.address);
