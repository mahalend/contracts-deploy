...
import { checkRequiredEnvironment, ConfigNames, getChainlinkOracles, getParamPerNetwork, getReserveAddresses, loadPoolConfig } from "../../helpers/market-config-helpers";
...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...hre }: HardhatRuntimeEnvironment) {
    ...
    const fallbackOracleAddress = (await getParamPerNetwork(poolConfig.FallbackOracle, network as eNetwork)) ||
        ZERO_ADDRESS;
    const reserveAssets = await getReserveAddresses(poolConfig, network as eNetwork);
    const chainlinkAggregators = await getChainlinkOracles(poolConfig, network as eNetwork);
